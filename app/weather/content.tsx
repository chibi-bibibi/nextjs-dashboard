...
const Content = () => {
  ...
  const [selectedPref, setSelectedPref] = useState<string>(prefecture[0].prefectures[0].name);
  const [selectedCity, setSelectedCity] = useState<string>(city[0].cities[0].id);
  ...
  const onClick = async (selectedCity: string | undefined) => {
    const res = await fetch(`https://weather.tsukumijima.net/api/forecast/city/${selectedCity}`, {
      mode: 'cors',
    });
  ...
  return (
    <>
      <div className="my-5 flex flex-col items-center justify-center text-base font-bold text-blue ">
        ...
        <p className="text-xs">※APIの仕様上、当日の気温と降水確率は取得できません</p>
      </div>
      ...
      {forecast && (
        <div className="items-center justify-center px-20 sm:flex ">
          <div className="h-full justify-center sm:flex sm:w-4/5 bg-de">
            <div className="my-5 flex h-96 flex-col rounded border-2 border-red mx-10 sm:w-1/3 bg-default_light items-center">
              ...
              <p className="mt-2 text-xl text-blue">{forecast.today.temperature.min.celsius}/{forecast.today.temperature.max.celsius}</p>
              <p className="mt-5 text-2xl text-blue">降水確率</p>
              <p className="mt-2 text-xl text-blue">00時 ~ 06時： {forecast.today.chanceOfRain.T00_06}</p>
              <p className="text-xl text-blue">06時 ~ 12時： {forecast.today.chanceOfRain.T06_12}</p>
              <p className="text-xl text-blue">12時 ~ 18時： {forecast.today.chanceOfRain.T12_18}</p>
              <p className="text-xl text-blue">18時 ~ 24時： {forecast.today.chanceOfRain.T18_24}</p>
            </div>
            ...
            <div className="my-5 flex h-96 flex-col rounded border-2 border-red mx-10 sm:w-1/3 bg-default_light items-center">
              ...
              />
              <p className="mt-5 text-2xl text-blue">気温（最低/最高）</p>
              <p className="mt-2 text-xl text-blue">{forecast.tomorrow.temperature.min.celsius}/{forecast.tomorrow.temperature.max.celsius}</p>
              <p className="mt-5 text-2xl text-blue">降水確率</p>
              <p className="mt-2 text-xl text-blue">00時 ~ 06時： {forecast.tomorrow.chanceOfRain.T00_06}</p>
              <p className="text-xl text-blue">06時 ~ 12時： {forecast.tomorrow.chanceOfRain.T06_12}</p>
              <p className="text-xl text-blue">12時 ~ 18時： {forecast.tomorrow.chanceOfRain.T12_18}</p>
              <p className="text-xl text-blue">18時 ~ 24時： {forecast.tomorrow.chanceOfRain.T18_24}</p>
            </div>
            ...
            <div className="my-5 flex h-96 flex-col rounded border-2 border-red mx-10 sm:w-1/3 bg-default_light items-center">
              ...
              />
              <p className="mt-5 text-2xl text-blue">気温（最低/最高）</p>
              <p className="mt-2 text-xl text-blue">{forecast.future.temperature.min.celsius}/{forecast.future.temperature.max.celsius}</p>
              <p className="mt-5 text-2xl text-blue">降水確率</p>
              <p className="mt-2 text-xl text-blue">00時 ~ 06時： {forecast.future.chanceOfRain.T00_06}</p>
              <p className="text-xl text-blue">06時 ~ 12時： {forecast.future.chanceOfRain.T06_12}</p>
              <p className="text-xl text-blue">12時 ~ 18時： {forecast.future.chanceOfRain.T12_18}</p>
              <p className="text-xl text-blue">18時 ~ 24時： {forecast.future.chanceOfRain.T18_24}</p>
            </div>
